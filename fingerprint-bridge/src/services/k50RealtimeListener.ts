...
const UDP_PORTS = config.k50.enabled ? config.k50.udpPorts : [5000, 3070, 3000]
...
function parseAttendancePacket(buffer: Buffer): {
  ...
  // Header: DWORD command = 0x0050 (or 0x00000050 LE, or byte-swapped 0x50000000)
  if (buffer.length >= 37) {
    const command = buffer.readUInt32LE(0)

    // Check for common attendance event commands (handling endianness)
    const isAttEvent = command === 0x50 || command === 0x50000000
    if (isAttEvent) {
      const userId = extractUserId(buffer, 8, 24)
      const ts = extractTimestamp(buffer, 32)

      if (userId && ts) {
        const eventType = buffer[36] === 0x01 ? 'checkout' : 'checkin'
        return { userId, timestamp: ts, eventType }
      }
    }
  }
...
function handleMessage(msg: Buffer, rinfo: dgram.RemoteInfo) {
  ...
  const timeStr = new Date(parsed.timestamp * 1000).toISOString()
  logger.info(
    `[K50-UDP] ✓ Attendance event: user="${parsed.userId}" time="${timeStr}" type="${parsed.eventType || 'checkin'}"`
  )
...
export async function startK50Listener(): Promise<void> {
  ...
  server.on('error', (err) => {
    logger.error(`[K50-UDP] Socket error: ${err.message} — restarting listener in 10s`)
    server = null
    listening = false
    // Auto-restart after a delay (with guard to prevent duplicate restarts)
    if (!restartTimer) {
      restartTimer = setTimeout(() => {
        restartTimer = null
        startK50Listener().catch((e) =>
          logger.error(`[K50-UDP] Restart failed: ${e.message}`)
        )
      }, 10000)
    }
  })
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        ...
        if (!bound) {
          ...
          // Already bound to the first port — skip remaining ports.
          // A single UDP socket can only bind to one port. To listen on
          // multiple ports, create additional sockets (add logic here if needed).
          resolve()
        }
        s.once('error', reject)
